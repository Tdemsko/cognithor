...
Loads configuration from:
  ...
  2. ~/.cognithor/config.yaml (overrides defaults)
  3. Environment variables COGNITHOR_* (overrides everything)

Automatically creates the ~/.cognithor/ directory structure on first start.
...
class EmailConfig(BaseModel):
    ...
    password_env: str = "COGNITHOR_EMAIL_PASSWORD"
...
class CalendarConfig(BaseModel):
    ...
    ics_path: str = ""
    """Pfad zur lokalen ICS-Datei (Default: ~/.cognithor/calendar.ics)."""
    ...
    password_env: str = "COGNITHOR_CALENDAR_PASSWORD"
...
class MarketplaceConfig(BaseModel):
    ...
    db_path: str = Field(
        default="",
        description="Pfad zur Marketplace-DB (leer = ~/.cognithor/marketplace.db)",
    )
    """Pfad zur SQLite-Datenbank. Wenn leer, wird ``~/.cognithor/marketplace.db``
    verwendet."""
...
class MtlsConfig(BaseModel):
    ...
    certs_dir: str = Field(
        default="", description="Zertifikats-Verzeichnis (Standard: ~/.cognithor/certs/)"
    )
...
class AuditConfig(BaseModel):
    ...
    ed25519_key_file: str = Field(
        default="",
        description="Pfad zur Ed25519-Key-Datei (leer = ~/.cognithor/audit_ed25519.key)",
    )
...
class ArcConfig(BaseModel):
    ...
    recording_dir: str = Field(default="~/.cognithor/recordings/arc")
...
class CognithorConfig(BaseModel):
    ...
    language: str = Field(
        ...
        "Also settable via COGNITHOR_LANGUAGE env var.",
    )
    ...
    def ensure_directories(self) -> list[str]:
        """Erstellt ~/.cognithor/ Verzeichnisstruktur."""
...
def load_config(config_path: Path | None = None) -> CognithorConfig:
    ...
    Order (later overrides earlier):
      ...
      3. COGNITHOR_* Umgebungsvariablen
...
# Heartbeat checklist -- created on first start in ``~/.cognithor/HEARTBEAT.md``
